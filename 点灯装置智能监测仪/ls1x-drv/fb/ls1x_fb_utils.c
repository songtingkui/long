 ...
 * 输出文本
 */
int fb_textout(int x, int y, char *str)
{
	char *pch = str;
	int   dx = 0;
...
int fb_put_string(int x, int y, char *str, unsigned coloridx)
{
	unsigned int dx, saved_coloridx = fb->fg_coloridx;
	fb->fg_coloridx = coloridx;
	dx = fb_textout(x, y, str);
...
返回值：    无
******************************************************************/
static void draw_circle_8(int xc, int yc, int x, int y, unsigned c)
{
    //于坐标（xc + x， yc + y）画点
    fb_drawpixel(xc + x, yc + y, c);
...
返回值： 无
******************************************************************/
void fb_circle(int xc, int yc, int r, int fill, unsigned coloridx)
{
    ...
    if (fill)                       // 填充式画圆（实心圆）
    {
        ...
        while (x <= y)
        {
            ...
            {
                // 8对称性画圆，一次画八个点
                draw_circle_8(xc, yc, x, yi, coloridx);
            }
    ...
    else                            // 非填充式画圆（空心圆）
    {
        ...
        {
            // 8对称性画圆，一次画八个点
            draw_circle_8(xc, yc, x, y, coloridx);
...
static void LS1x_draw_rgb565_pixel(unsigned int x, unsigned int y, unsigned int color)
{
    unsigned int fbAddr;
 ...
 * dispplay bmp(rgb565)
 */
void fb_displaybmp(unsigned int xpos,unsigned int ypos,unsigned int x1,unsigned int y1,const unsigned char *ptrs)
{
    {
        int x, y;
        unsigned char *ptr = (unsigned char *)ptrs;
