...
static unsigned int NS_2_BUS_CLOCKS(unsigned int bus_freq, unsigned int ns)
{
    ...
    clocks = (unsigned int)(((unsigned long long)ns * bus_freq) / 1000000000UL);
...
#define PWM_TIMER_CLEAR(pwm) \
        do { \
            pwm->hwPWM->ctrl = pwm_ctrl_cntr_en | pwm_ctrl_iflag | \
                               pwm_ctrl_oe_mask; \
            ls1x_sync(); \
        } while (0)
...
static void LS1x_PWM_timer_common_isr(int vector, void *arg)
{
    ...
    if (!pwm->single)
    {
        PWM_TIMER_CLEAR(pwm);       /* Clear PWM int mark */
        PWM_TIMER_START(pwm);
    }
