...
extern int fb_textout(int x, int y, char *str);                /* 在LCD[x,y]处打印字符串 */

//-----------------------------------------------------------------------------
// 显示 BMP图像
//-----------------------------------------------------------------------------
extern void fb_displaybmp(unsigned int xpos, unsigned int ypos, unsigned int x1, unsigned int y1, const unsigned char *ptrs);
 ...
 */
extern int fb_put_string(int x, int y, char *str, unsigned coloridx);
...
/* 在LCD[xc,yc]处用指定颜色画画圆
 */
extern void fb_circle(int xc, int yc, int r, int fill, unsigned coloridx);
